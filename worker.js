...
// Handle Hugo export functionality
if (url.pathname === '/export/hugo' && request.method === 'GET') {
    // Fetch the latest article data
    const keys = await KV_STORAGE.list();
    if (keys.keys.length === 0) {
        return new Response('No articles found', { status: 404 });
    }

    const key = keys.keys[keys.keys.length - 1].name; // Fetch the latest article
    const data = await KV_STORAGE.get(key);

    if (!data) {
        return new Response('Content not found', { status: 404 });
    }

    const { title, date, tags, categories, content } = JSON.parse(data);

    // Construct Hugo Front Matter
    const frontMatter = `---
title: "${title}"
date: "${date}"
...
`;
    ...
    const hugoMarkdown = `${frontMatter}\n\n${content}`;

    return new Response(hugoMarkdown, {
        headers: {
            'Content-Type': 'text/markdown; charset=utf-8',
            'Content-Disposition': 'attachment; filename="hugo_article.md"',
        },
    });
}
